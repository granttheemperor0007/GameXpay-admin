...
import type { Game, Bundle } from '../types';
...
function BundleRow({ bundle, onChange, onRemove, isSuperAdmin, index }: BundleRowProps) {
...
function GameFormModal({ isOpen, onClose, game, onSaved }: GameFormModalProps) {
  ...
  const bundlesEndRef = useRef<HTMLDivElement>(null);
    ...
    if (isOpen) {
      if (game) {
        setForm({
          ...
          status: game.status,
          bundles: game.bundles.map((b) => ({ ...b, sellingPrice: b.sellingPrice, costPrice: b.costPrice })),
        });
  ...
  const updateBundle = (idx: number, field: string, value: string) => {
    setForm((f) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isEdit && game) {
        ...
        await gamesService.create(payload as Omit<Game, 'id' | 'createdAt'>);
      }
      onSaved();
      onClose();
    } finally {
            ...
            <div className="flex items-center gap-3">
              {imagePreview && (
                <img
                  ...
                  onError={(e) => { e.currentTarget.src = 'https://placehold.co/24x24/1e1e1e/ffffff?text=?'; }}
                />
              )}
...
export default function Games() {
  ...
  const [formModal, setFormModal] = useState<{ open: boolean; game: Game | null }>({ open: false, game: null });
  const [toggleConfirm, setToggleConfirm] = useState<{ open: boolean; game: Game | null }>({ open: false, game: null });
  ...
  useEffect(() => {
    const editGameId = (location.state as { editGameId?: string } | null)?.editGameId;
  ...
  const columns = [
    {
      ...
      label: '',
      width: '56px',
      render: (val: unknown, row: Game) => (
        <img
          src={val as string}
          alt={row.name}
          className="w-6 h-6 rounded object-cover"
          onError={(e) => { e.currentTarget.src = `https://placehold.co/24x24/1e1e1e/ffffff?text=${row.shortName}`; }}
        />
      ),
    },
    {
      ...
      render: (val: unknown, row: Game) => (
        <div>
          <p className="font-medium text-gray-200">{val as string}</p>
    ...
    },
    {
      ...
      render: (val: unknown) => (
        <div className="flex items-center gap-1.5 text-gray-400">
          <Package size={13} />
          <span>{(val as Bundle[] | undefined)?.length ?? 0}</span>
        </div>
      ),
    },
    {
      key: 'createdAt',
      label: 'Date Created',
      render: (val: unknown) => <span className="text-gray-400">{fmtDate(val as string)}</span>,
    },
    {
      key: 'status',
      label: 'Status',
