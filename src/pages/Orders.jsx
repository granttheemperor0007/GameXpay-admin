...
// ─── Order Detail Modal ────────────────────────────────────────────────────────
function OrderDetailModal({ order, isOpen, onClose, onStatusChange, onNoteSaved, isSuperAdmin }) {
  const [note, setNote] = useState('');
  const [editNote, setEditNote] = useState(false);
  const [savingNote, setSavingNote] = useState(false);
  const [actionLoading, setActionLoading] = useState(null); // 'complete' | 'cancel'
...
export default function Orders() {
  ...
  const handleNoteSaved = (id, note) => {
    ...
    if (selectedOrder?.id === id) {
      setSelectedOrder((o) => ({ ...o, notes: note }));
    }
  };
  ...
  const columns = [
    { key: 'id', label: 'Order ID', render: (val) => <span className="font-mono text-xs text-violet-400">{val}</span> },
    { key: 'status', label: 'Status', render: (val) => <Badge status={val} /> },
    { key: 'customerEmail', label: 'Customer Email', render: (val) => <span className="text-xs">{val}</span> },
    { key: 'playerID', label: 'Player ID', render: (val) => <span className="font-mono text-xs">{val}</span> },
    {
      key: 'game',
      label: 'Product',
      render: (val, row) => (
        <div>
          <p className="text-xs font-medium text-gray-200">{val?.name}</p>
